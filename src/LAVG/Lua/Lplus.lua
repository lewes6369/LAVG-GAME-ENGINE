...
local function createType (baseTypeTable, typeNameOrForwardDeclare, isInterface)
	...
	--[[
		...
		declare a static field
			param fieldType: type of field
		e.g: const("number").m_fieldName = 1
	]]
...
--[[
	...
	只是提前声明类，类可以尚未定义，在定义类时再把声明表中的对应项去除
]]
...
--[[
	Create a class with Object to be it's base type
		param typeNameOrForwardDeclare:
			...
			when is a table: anonymous forward declared type (return value by Lplus.ForwardDeclare)  --这个没太理解 意味参数即为前向类型的typetable? 然则前向类型的typetable如何获得？
